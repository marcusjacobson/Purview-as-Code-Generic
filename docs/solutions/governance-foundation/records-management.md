...
> **No automated apply path yet.** No per-solution workflow owns records management / the file plan, so merging `data-plane/records/**` applies nothing on its own. **Interim apply path: run [`scripts/Deploy-FilePlan.ps1`](../../../scripts/Deploy-FilePlan.ps1) locally.** The monolithic `deploy-data-plane.yml` that once carried a `Deploy file plan` step (inputs `records_direction_policy`, `confirm_overwrite_records`, `skip_names_records`) was retired by [ADR 0051](../../adr/0051-per-solution-workflow-unit-of-data-plane-apply.md) — it declared 32 `workflow_dispatch` inputs against GitHub's 25-property cap and therefore **never once executed** (90 runs, 0 successes, 0 jobs scheduled), so that step never applied anything. Nothing was lost. Backfilling a `deploy-file-plan.yml` is tracked in [#80](https://github.com/marcusjacobson/Purview-as-Code/issues/80).

> [!WARNING]
> **The seed-name skip baseline was a *workflow input default*, and that default is now gone.** The retired workflow's `skip_names_records` input carried the **29 unique seed names** (covering all 31 Microsoft File Plan Manager seed property objects, since `Legal` and `Procurement` each appear under two kinds) ratified as immovable in [ADR 0035](../../adr/0035-records-seed-content-immovable.md), which made every dispatch noise-free without operator intervention. **Running the reconciler locally, nothing supplies that list for you** — you must pass `-SkipNames` explicitly from the ADR 0035 table, which is now its **sole** source of truth. See the ADR 0035 section below for the consequence of forgetting.

The script-side ADR 0029 contract is unaffected: pass `-DirectionPolicy {audit, portal-wins, repo-wins}` on the local command line. **The typed `overwrite portal` confirmation that gated `repo-wins` was a workflow pre-flight step, not a script parameter** — locally, `-DirectionPolicy repo-wins` is destructive with no prompt, so preview with `-DirectionPolicy audit` first. With desired state empty and the seed baseline skipped, the apply is a Skipped-only no-op; the contract is scaffolding for the day the first operator-authored label or property is declared. If Microsoft ever shrinks or grows the seed list in a service revision, update the workflow default, [ADR 0035](../../adr/0035-records-seed-content-immovable.md)'s source-of-truth table, and the [runbook](../../runbooks/records-end-to-end-smoke.md) together.
...
Every Microsoft 365 tenant carries 31 Microsoft-shipped seed property objects (3 authorities, 13 categories, 5 citations, 10 departments) that `Remove-FilePlanProperty*` rejects via every documented identity form (`Name`, `Guid`, `CN=<guid>`) with `ErrorRuleNotFoundException`. [ADR 0035](../../adr/0035-records-seed-content-immovable.md) ratifies treating these as permanent declared orphans, skipped via `-SkipNames`.

**The full verbatim 31-name table in [ADR 0035](../../adr/0035-records-seed-content-immovable.md) is now the *sole* source of truth.** It previously had an operational copy in the retired workflow's `skip_names_records` input default; that copy is gone with the workflow ([ADR 0051](../../adr/0051-per-solution-workflow-unit-of-data-plane-apply.md)), and no default supplies the list to a local run. **Never `-PruneMissing` without `-SkipNames`** against `contoso.onmicrosoft.com` — the seed prune will produce 31 `Failed` rows per the [#582](../../../../issues/582) post-mortem. This warning is now *load-bearing* rather than belt-and-braces: the workflow default that used to make it hard to get wrong no longer exists.
