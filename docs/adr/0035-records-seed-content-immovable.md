...
> **Update 2026-07-18 (provenance resolved, not a decision change — issue [#98](https://github.com/marcusjacobson/Purview-as-Code/issues/98)).**
>
> The `Finance` department's `Guid` (`5ff13e11-12ad-466d-bf6e-a2c9966fb36e`) and `Policy` GUID (`c25c0d36-48f0-45d8-babe-521127b3fa96`) captured in the probe transcript below (§Context, "Probe against `contoso.onmicrosoft.com`") sat in [`.github/agents/tenant-placeholders.yaml`](../../.github/agents/tenant-placeholders.yaml)'s `identifierScan.reviewRequired` quarantine from [#94](https://github.com/marcusjacobson/Purview-as-Code/issues/94)/[#97](https://github.com/marcusjacobson/Purview-as-Code/pull/97) onward: this ADR's line 61 *inferred* they were Microsoft-managed but that inference was never verified, so the resolver declined to launder it into `identifierScan.microsoftConstants`.
>
> **Provenance is now established.** `Get-FilePlanPropertyDepartment` was run against a second, independently-provisioned Microsoft 365 tenant, distinct from the tenant this ADR's original probe used. Result: the `Finance` department's `Guid` was identical to the value recorded below, and the `Policy` GUID was not only identical but shared across all ten seeded departments in the second tenant too — the signature of a hardcoded Microsoft-shipped constant applied uniformly to seed content, not a tenant-generated value. Both GUIDs are promoted to `identifierScan.microsoftConstants` in the manifest, with the empirical cross-tenant methodology recorded as the citation (no Microsoft Learn page indexes either literal value). The `reviewRequired` quarantine is now empty, and `.github/workflows/validate.yml` runs `Test-IdentifierResidue.ps1 -FailOnReview`, so the scan fails closed with no standing exceptions. This does not change the Decision above: the 31 seeds remain undeletable, declared orphans; only the provenance of two incidentally-disclosed GUIDs in this ADR's probe transcript was resolved.
...
Watch-list issue [#3](../../issues/3) flagged this entry: the tracked page's `updated_at` advanced from the recorded baseline (`2026-04-30`) to `2026-06-25`. Note: this is the *programmatic-removal* watch-list question (§Decision item 6's "Re-open triggers"), distinct from — and not to be conflated with — the separate GUID-provenance question already resolved in issue [#98](../../issues/98) (see the "Update 2026-07-18 (provenance resolved...)" note at the top of this ADR). Re-verified the re-open triggers against fresh evidence on 2026-07-18.

| # | Trigger (from §Re-open triggers) | Status | Evidence (2026-07-18) |
|---|---|---|---|
| 1 | [Remove-FilePlanPropertyAuthority](https://learn.microsoft.com/en-us/powershell/module/exchangepowershell/remove-fileplanpropertyauthority) (or any sibling `Remove-FilePlanProperty*` page) documents a `-Policy` parameter or any other parameter that targets the Microsoft-managed seed policy scope. | **Cold.** | The page's full parameter list (fetch 2026-07-18, updated_at 2026-05-16): `-Identity`, `-Confirm`, `-ForceDeletion`, `-WhatIf`, plus common parameters. No `-Policy`. |
| 2 | A `filePlanProperty`, `recordLabel`, or similarly-named resource lands under `https://learn.microsoft.com/en-us/graph/api/resources/` (beta or v1.0) with a `DELETE` endpoint. | **Cold.** | The Graph `security-fileplandescriptor` resource (fetched during the #98 provenance research) explicitly lists `## Methods` as `None` — no CRUD operations at all are documented for that resource. |
| 3 | [Records management overview](https://learn.microsoft.com/en-us/purview/records-management) or [File plan manager](https://learn.microsoft.com/en-us/purview/file-plan-manager) gains a "programmatically remove the default seed content" section. | **Cold.** | [Records management overview](https://learn.microsoft.com/en-us/purview/records-management) (fetch 2026-07-18, updated_at 2026-06-25) is entirely label / retention / disposition-focused with zero mention of removing seed / default content. The tracked page, [File plan manager](https://learn.microsoft.com/en-us/purview/file-plan-manager) itself (fetch 2026-07-18, updated_at 2026-06-25 — the update that triggered this flag), only documents CSV import/export, not programmatic seed deletion. |
| 4 | A Microsoft-published reference repo ships a sample that deletes seeded file plan property objects via a documented surface. | **Not independently re-searched this pass.** | Consistent with findings 1-3 above. |
| 5 | The portal-only removal path is verified end-to-end against `contoso.onmicrosoft.com` and confirmed to delete one of the 31 seeds without violating Microsoft support terms. | **Untested — unchanged.** | This is an operator tenant-action trigger, not a Learn-page trigger; it cannot be verified via documentation research and is out of scope for this documentation-only re-verification pass. Not marked fired or not-fired — left as not-yet-attempted, matching this ADR's original state. |

**Verdict: zero of the four documentation-verifiable triggers fired; the fifth (tenant-action) trigger remains untested (unchanged).** The 2026-06-25 update to the tracked page and to [Records management overview](https://learn.microsoft.com/en-us/purview/records-management) did not introduce a programmatic seed-removal path. Standing decision unchanged: the 31 seeds remain permanent, declared orphans; `-SkipNames` baseline in [`data-plane/records/seed-skip-names.yaml`](../../data-plane/records/seed-skip-names.yaml) unchanged. Project plan §8 Q15 stays unticked — it is a standing watch-list row by design ("permanently open until any re-open trigger below fires"), per §Decision item 5 above. `docs/adr/watch-list.yml`'s `adr-0035-q15-records-file-plan-manager` entry is re-baselined to `baseline: '2026-06-25'`, `lastVerified: '2026-07-18'`.
