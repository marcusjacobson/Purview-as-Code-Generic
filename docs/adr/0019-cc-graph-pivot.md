...
Watch-list issue [#3](../../issues/3) flagged this entry: the tracked page's `updated_at` advanced from the recorded baseline (`2026-06-01`) to `2026-06-23`. Re-verified all four §Decision item 5 re-open triggers against fresh evidence on 2026-07-18.

| # | Trigger (from §Decision item 5) | Status | Evidence (2026-07-18) |
|---|---|---|---|
| 1 | A `communicationCompliance` resource lands under `https://learn.microsoft.com/en-us/graph/api/resources/` (beta or v1.0). | **Cold.** | [Microsoft Graph security API overview](https://learn.microsoft.com/en-us/graph/api/resources/security-api-overview) (fetch 2026-07-18, updated_at 2026-05-11) — the "Records management" section lists only `retentionLabel` / `fileplandescriptor`; no Communication Compliance or `supervisoryReview` resource anywhere on the page. |
| 2 | The Communication Compliance solution overview or Configure Communication Compliance page gains a programmatic-authoring section. | **Cold — and reinforced.** | [Configure Communication Compliance](https://learn.microsoft.com/en-us/purview/communication-compliance-configure) (fetch 2026-07-18, updated_at 2025-12-17): *"You can't use PowerShell to create and manage Communication Compliance policies. You must use the Communication Compliance solution."* The tracked page itself, [Create and manage Communication Compliance policies](https://learn.microsoft.com/en-us/purview/communication-compliance-policies) (fetch 2026-07-18, updated_at 2026-06-23 — the very update that triggered this flag), states even more explicitly: *"PowerShell isn't supported for creating and managing Communication Compliance policies. To create and manage these policies, use the policy management controls in the Communication Compliance solution."* |
| 3 | Microsoft adds a deprecation notice to `New-SupervisoryReviewPolicyV2` naming a successor. | **Cold.** | [New-SupervisoryReviewPolicyV2](https://learn.microsoft.com/en-us/powershell/module/exchangepowershell/new-supervisoryreviewpolicyv2) (fetch 2026-07-18, updated_at 2026-05-16) carries no deprecation notice and names no successor; parameter set unchanged (`-Name`, `-Reviewers`, `-Comment`, `-Confirm`, `-Enabled`, `-Force`, `-PolicyRBACScopes`, `-PreservationPeriodInDays`, `-UserReportingWorkloads`, `-WhatIf`). |
| 4 | A Microsoft-published reference repo ships a CC policy authoring sample. | **Not independently re-searched this pass.** | Consistent with, and made extremely unlikely by, findings 1-3 above — especially the tracked page's own explicit "PowerShell isn't supported" statement dated to the same update that triggered this flag. |

**Verdict: zero of the four triggers fired.** The 2026-06-23 update to the tracked page was a routine content refresh (word-count / AI-usage metadata churn), not a capability change — if anything it reinforces the existing "portal-only" position with a more explicit callout than the 2026-06-07 re-verification found. Standing rule unchanged: `policies: []` in [`data-plane/communication-compliance/policies.yaml`](../../data-plane/communication-compliance/policies.yaml), reconciler retained for drift detection only. Project plan §8 Q10 stays unticked — it is a standing watch-list row by design, not a one-time resolvable item, per §Decision item 4 above. `docs/adr/watch-list.yml`'s `adr-0019-q10-comms-compliance` entry is re-baselined to `baseline: '2026-06-23'`, `lastVerified: '2026-07-18'`.
