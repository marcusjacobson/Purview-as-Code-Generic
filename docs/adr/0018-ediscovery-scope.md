...
The Progress checklist Open-question ADRs sub-section of the v1 plan ([`docs/archive/project-plan-v1.md`](../archive/project-plan-v1.md#open-question-adrs)) carried Q6 — "Is eDiscovery (cases, holds, exports) in scope for this lab's data-plane-as-code?" — as the gate for Wave 2f. Wave 2f's planned shape ([#82](../../issues/82)) was `data-plane/ediscovery/cases.yaml` plus `Deploy-eDiscovery.ps1`, mirroring the YAML-plus-reconciler pattern used by every other Wave 1 and Wave 2 item.
