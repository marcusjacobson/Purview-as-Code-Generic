...
.PARAMETER DirectionPolicy
    ...
    desired YAML and the live tenant. One of:
      ...
      * `repo-wins`   -- apply the full plan including shared-property
                         ...
                         data sources it is about to overwrite, asks EVERY
...
.PARAMETER OverwriteForeignAuthor
    Apply parameter set only. Permit `Update` writes against tenant data
    sources whose authorship (`lastModifiedBy` / `updatedBy` /
...
# ADR 0052: every data source whose tenant fields this run WILL overwrite.
...
# the overwrite gate AND the prune gate prompts once, not twice.
#
...
if ($repoWinsOverwrites.Count -gt 0) {
    ...
    $overwriteQuery = "This run will OVERWRITE tenant fields on {0} Purview data source(s) with the values from YAML: {1}. Portal edits to those fields are lost. Continue?" -f `
...
if ($PruneMissing.IsPresent -and $pruneTargets.Count -gt 0) {
    ...
    $pruneQuery = "-PruneMissing will DELETE {0} orphan Purview data source(s) from the account: {1}. This cannot be undone. Continue?" -f `
