...
<#
.SYNOPSIS
    Near-unattended wrapper around docs/runbooks/data-sources-end-to-end-smoke.md
    for capturing v2 §5.5 Purview Data Map Data Sources evidence in one command.
...
.DESCRIPTION
    ...
    Throwaway data source named e2e-data-sources-smoke-<YYYYMMDD-HHmm>,
    ...
    .copilot-tracking/smoke/data-sources-<UTC>.md that the operator pastes
    verbatim into the v2 §5.5 row 2 close-out PR.

    Hard rule: pre-existing live data sources in the tenant MUST NOT be
    mutated. The wrapper enforces this by:

      - Only ever issuing DELETE on names matching ^e2e-data-sources-smoke-
        (asserted before every DELETE call).
      - Asserting that every Deploy-DataSources.ps1 -WhatIf plan classifies
        each pre-existing live data source as NoChange (never Update / Failed).
    ...
    PUT/GET/DELETE plus scripts/Deploy-DataSources.ps1.
    ...
    References:
      ...
      - docs/runbooks/data-sources-end-to-end-smoke.md
...
.PARAMETER SmokeDataSourceName
    Name of the Throwaway data source. Defaults to
    e2e-data-sources-smoke-<YYYYMMDD-HHmm>. Must start with
    'e2e-data-sources-smoke-' to keep accidental real-collection
    collisions impossible.

.PARAMETER PurviewAccountName
    Purview account to target. Defaults to resolution by
    scripts/Deploy-DataSources.ps1 from infra/parameters/lab.yaml.
...
.EXAMPLE
    PS> ./scripts/Invoke-DataSourcesSmokeTest.ps1
...
param(
    ...
    [Parameter()][ValidatePattern('^e2e-data-sources-smoke-[a-z0-9-]+$')][string]$SmokeDataSourceName,
...
if (-not $SmokeDataSourceName) {
    $SmokeDataSourceName = "e2e-data-sources-smoke-$((Get-Date).ToString('yyyyMMdd-HHmm'))"
}
...
function Assert-SmokePrefix {
    param([string]$Name)
    if ($Name -notmatch '^e2e-data-sources-smoke-') {
        throw "Safety assert failed: '$Name' does not start with 'e2e-data-sources-smoke-'. Refusing to issue DELETE."
    }
}
...
$deployScript = Join-Path $PSScriptRoot 'Deploy-DataSources.ps1'
$connectScript = Join-Path $PSScriptRoot 'Connect-Purview.ps1'
if (-not (Test-Path -LiteralPath $deployScript)) {
    Add-Result -Step 0 -Title 'Preconditions: Deploy-DataSources.ps1' -Result 'FAIL' -Reason "Not found at $deployScript"
    exit 2
}
...
$gitStatus = (git status --short data-plane/data-sources/ 2>$null) -join ''
if (-not [string]::IsNullOrWhiteSpace($gitStatus)) {
    Add-Result -Step 0 -Title 'Preconditions: clean tree under data-plane/data-sources/' -Result 'FAIL' -Reason "git status -s returned: $gitStatus"
    exit 2
}
...
if (-not $PurviewAccountName) {
    ...
    # Deploy-DataSources.ps1 does, so this wrapper does not depend on
...
$endpointHost = "e2esmoke$(($SmokeDataSourceName -replace '[^a-z0-9]','').Substring([math]::Max(0,($SmokeDataSourceName -replace '[^a-z0-9]','').Length - 16)))"
$createUri = "$baseUri/datasources/$([uri]::EscapeDataString($SmokeDataSourceName))?api-version=$script:DataSourcesApiVersion"
# AzureStorage kind requires a populated resourceId (the registration
# does not validate connectivity, but the Purview REST surface 404s on
# a bare body). The placeholder rg / storage-account name never exists
# in Azure; nothing scans it; it is torn down within seconds.
$subId = [Environment]::GetEnvironmentVariable('AZURE_SUBSCRIPTION_ID')
if ([string]::IsNullOrEmpty($subId)) {
    Add-Result -Step 2 -Title 'Create throwaway' -Result 'FAIL' -Reason 'AZURE_SUBSCRIPTION_ID env var must be set so the throwaway resourceId resolves. See docs/adr/0023-identifier-resolution.md.'
    exit 2
}
$createBody = @{
    kind       = 'AzureStorage'
    properties = @{
        ...
        resourceId        = "/subscriptions/$subId/resourceGroups/rg-purview-lab/providers/Microsoft.Storage/storageAccounts/$endpointHost"
        location          = 'eastus'
...
try {
    ...
    Add-Result -Step 2 -Title 'Create throwaway' -Result 'PASS' -Reason ("PUT created '{0}' (kind={1})." -f $created.name, $created.kind) | Out-Null
} catch {
...
if ($step3Output -match "(?m)^Orphan\s+DataSource\s+$([regex]::Escape($SmokeDataSourceName))\b") {
    Add-Result -Step 3 -Title 'Orphan reported' -Result 'PASS' -Reason ("'{0}' appears as Orphan in WhatIf plan." -f $SmokeDataSourceName) | Out-Null
} else {
    Add-Result -Step 3 -Title 'Orphan reported' -Result 'FAIL' -Reason ("'{0}' not found as Orphan. Output tail: {1}" -f $SmokeDataSourceName, ($step3Output.Split("`n") | Select-Object -Last 10 | Out-String))
}
...
$step4Output = & $deployScript -WhatIf -SkipNames @($SmokeDataSourceName) *>&1 | Out-String
if ($step4Output -match "\[ADR0029-SKIP\] $([regex]::Escape($SmokeDataSourceName))" -and
    $step4Output -match "(?m)^Skip\s+DataSource\s+$([regex]::Escape($SmokeDataSourceName))\b") {
    Add-Result -Step 4 -Title 'SkipNames suppresses orphan' -Result 'PASS' -Reason ("'{0}' appears as Skip with ADR0029-SKIP marker." -f $SmokeDataSourceName) | Out-Null
} else {
...
Assert-SmokePrefix -Name $SmokeDataSourceName

if (-not $SkipDestructiveConfirmation) {
    Write-Information '' -InformationAction Continue
    Write-Warning "About to DELETE Purview data source '$SmokeDataSourceName' from $PurviewAccountName."
...
$deleteUri = "$baseUri/datasources/$([uri]::EscapeDataString($SmokeDataSourceName))?api-version=$script:DataSourcesApiVersion"
try {
...
if ($gone) {
    Add-Result -Step 5 -Title 'Destructive cleanup' -Result 'PASS' -Reason ("'{0}' deleted; follow-up GET returns not-found." -f $SmokeDataSourceName) | Out-Null
} else {
    Add-Result -Step 5 -Title 'Destructive cleanup' -Result 'FAIL' -Reason ("'{0}' still resolvable after DELETE." -f $SmokeDataSourceName)
}
...
$evidencePath = Join-Path $EvidenceDirectory "data-sources-$utc.md"
$evidenceLines = New-Object 'System.Collections.Generic.List[string]'
$evidenceLines.Add("# Data sources end-to-end smoke evidence — $utc")
$evidenceLines.Add('')
...
$evidenceLines.Add("- Throwaway: ``$SmokeDataSourceName``")
$evidenceLines.Add("- Runbook: [docs/runbooks/data-sources-end-to-end-smoke.md](../../docs/runbooks/data-sources-end-to-end-smoke.md)")
$evidenceLines.Add('')
