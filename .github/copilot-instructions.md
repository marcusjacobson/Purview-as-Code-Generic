...
| `.github/prompts/` | Meta | Reusable prompt files invoked with `/` in Copilot Chat. Validation engine: `/build-item`. Operational: `/deploy-infra`, `/deploy-datamap`, `/discover-purview-account`, `/security-review`. Content interviews: `/add-classification`, `/add-data-source` (invoked by `@squad`). Session lifecycle: `/prepare-handoff`, `/resume-from-handoff`. | one `*.prompt.md` per workflow |
