...
| `.github/workflows/` | CI/CD | [`validate.yml`](workflows/validate.yml), [`deploy-infra.yml`](workflows/deploy-infra.yml), and one per-solution `deploy-<solution>.yml` per data-plane surface ([`deploy-labels.yml`](workflows/deploy-labels.yml), [`deploy-dlp.yml`](workflows/deploy-dlp.yml), …) | path filters bind each workflow to its plane, and each per-solution workflow to its one surface ([ADR 0051](../docs/adr/0051-per-solution-workflow-unit-of-data-plane-apply.md)) |
| `.github/instructions/` | Meta | Path-scoped Copilot rules (Bicep, PowerShell, YAML, workflows) | one file per domain |
| `.github/prompts/` | Meta | Reusable prompt files invoked with `/` in Copilot Chat. Validation engine: `/build-item`. Operational: `/deploy-infra`, `/deploy-datamap`, `/deploy-unified`, `/discover-purview-account`, `/security-review`. Content interviews: `/add-classification`, `/add-data-source` (invoked by `@squad`). Session lifecycle: `/prepare-handoff`, `/resume-from-handoff`. | one `*.prompt.md` per workflow |
...
- **New / changed catalog content (collection, term, classification, source, scan, policy)?** → `data-plane/` only. Deploys through the **per-solution** `deploy-<solution>.yml` that owns that surface ([ADR 0051](../docs/adr/0051-per-solution-workflow-unit-of-data-plane-apply.md)). If the surface has no per-solution workflow yet, there is **no automated apply path** — it is applied by running its `scripts/Deploy-*.ps1` reconciler locally (backfill tracked in [#80](https://github.com/marcusjacobson/Purview-as-Code/issues/80)).
- **New / changed apply logic?** → `scripts/` only. This triggers the per-solution workflow that owns the reconciler you changed, if one exists; otherwise the reconciler is run locally.
